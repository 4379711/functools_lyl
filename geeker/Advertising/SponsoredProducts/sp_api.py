...
class Campaigns(Client):
    ...
    def create_campaigns(self, name, targeting_type, state, daily_budget: (int, float),
        ...
        if all_data_list is not None:
            return self.add_currency(self.make_request(interface, method='POST', payload=all_data_list))
        ...
        return self.add_currency(self.make_request(interface, method='POST', payload=data))
...
class ProductAds(Client):
    ...
    def list_product_ads(self, **params):
        ...
        payload = {
            ...
            'adGroupIdFilter': params.get('adGroupIdFilter'),
            'campaignIdFilter': params.get('campaignIdFilter'),
            'adIdFilter': params.get('adIdFilter'),
            'sku': params.get('sku'),
...
class Bid(Client):
    ...
    def create_keywords_bid_recommendations(self, adGroupId, matchType=None, keyword=None, keywords=None):
        """
                Note:
                   TargetingType  must set <manual>.
                   How to create it ,see class <Campaigns> .
        ...
        # interface = 'sp/sponsoredkeywords/bidRecommendations'

        MyTypeAssert.number_assert(adGroupId)
        if keywords is not None:
            keywords_list = keywords
        else:
            MyTypeAssert.str_assert(keyword)
            assert matchType in ['phrase', 'broad', 'exact']
            keywords_list = [{'keyword': keyword, 'matchType': matchType}]
    ...
    def get_bid_recommendations(self, params_data, ad_group=None, match_type=None, match_value=None):
        ...
        #
        # }
        return self.make_request(interface, method='POST', payload=params_data)
...
class Keywords(Client):
    ...
    def create_biddable_keywords(self, ad_group_id, campaign_id,
                                 ...
                                 state, bid=None, all_data_list=None,
                                 **params):
    ...
    def update_biddable_keywords(self, keyword_id, state=None, bid=None, all_data_list=None, **params):
    ...
    def create_negative_keywords(self, ad_group_id, campaign_id,
        ...
        assert state in ["enabled", "archived", "paused"]
    ...
    def get_ad_group_suggested_keywords(self, adGroupId, **params):
        interface = 'adGroups/{}/suggested/keywords'.format(adGroupId)
        payload = {
    ...
    def get_ad_group_suggested_keywords_ex(self, adGroupId, **params):
        interface = 'adGroups/{}/suggested/keywords/extended'.format(adGroupId)
        payload = {
            'maxNumSuggestions': params.get('maxNumSuggestions'),
            'adStateFilter': params.get('adStateFilter'),
    ...
    def get_asin_suggested_keywords(self, asinValue, **params):
        interface = 'asins/{}/suggested/keywords'.format(asinValue)
        MyTypeAssert.str_assert(asinValue)
        payload = {
            'maxNumSuggestions': params.get('maxNumSuggestions')
        }
        return self.make_request(interface, payload=payload)

    def bulk_get_asin_suggested_keywords(self, asins, **params):
...
class Targets(Client):
    ...
    def list_targeting_clause(self, **params):
        ...
        payload = {
            ...
            'campaignIdFilter': params.get('campaignIdFilter'),
            'adGroupIdFilter': params.get('adGroupIdFilter'),
            'targetIdFilter': params.get('targetIdFilter')
        }
        return self.make_request(interface, payload=payload)
    ...
    def update_targeting_clause(self, all_data_list=None, **params):
        interface = 'sp/targets'
    ...
    def get_targeting_refinements_categories(self, **params):
    ...
    def get_brand_recommendations(self, **params):
        # keyword ，categoryId not both user
    ...
    def create_negative_targeting_clauses(self, all_data_list=None, **params):
        interface = 'sp/negativeTargets'
        if all_data_list is not None:
            return self.make_request(interface, method='POST', payload=all_data_list)
        return self.make_request(interface, method='POST', payload=params)

    def update_negative_targeting_clauses(self, all_data_list):
        interface = 'sp/negativeTargets'
...
class Reports(Client):
    ...
    def request_report(self, recordType, reportDate, metrics, segment=None):
        # campaign_type 旧版api
        assert recordType in ['campaigns', 'adGroups', 'keywords', 'productAds', 'targets', "asins"]
        interface = 'sp/{}/report'.format(recordType)
        MyTypeAssert.other_assert(metrics, types=list)

        # report_date must format YYYYMMDD . and not older than 60 days .
        assert len(reportDate) == 8
        payload = {
    ...
    def request_asin_report(self, reportDate, metrics, segment=None):
    ...
    def download_report(self, url=None, report_id=None):
        # return self.make_request(url=url)

        interface = 'reports/{}/download'.format(report_id)
        resp = self.make_request(interface=interface)
...
class EntitySnapshots(Client):
    ...
    def request_snapshot(self, record_type, state_filter):
        ...
        assert state_filter in ['enabled', 'paused', 'archived', None]
...
class ManagerAccount(Client):
    VERSION = ''

    def managerAccounts(self):
        """
