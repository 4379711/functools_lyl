...
class Client(MixParams):
    ...
    def __init__(self, client_id,
        ...
        :param region:region
        :param profile_id:profiled id ,must not None except <Profiles.list_profiles> .
    ...
    def add_currency(self, response):
        ...
        if 200 <= status_code < 300:
            if isinstance(response_data, list):
                ...
                response_data["currency"] = self.currency
...
class Portfolios(Client):
    ...
    def create_portfolios(self, amount, policy, start_date,
                          name, state='enabled', end_date=None, all_data_list=None):

        interface = 'portfolios'
        if all_data_list is not None:
            return self.add_currency(self.make_request(interface, method='PUT', payload=all_data_list))
