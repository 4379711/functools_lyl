...
class ReportType:
    ...
    # Not in documents
    FINANCIAL_SUMMARY = '_GET_DATE_RANGE_FINANCIAL_SUMMARY_DATA_'
    FINANCIAL_TRANSACTION = '_GET_DATE_RANGE_FINANCIAL_TRANSACTION_DATA_'
...
class DictWrapper(object):
    def __init__(self, xml, rootkey=None):
        ...
        self._mydict = utils.XML2Dict(rootkey).fromstring(remove_namespace(xml))
        # no self._rootkey
    ...
    def parsed(self):
        if self._rootkey:
            return self._mydict.get(self._rootkey)
...
class MWS(object):
    ...
    def make_request(self, extra_data, method="GET", **kwargs):
        ...
        """
        timeout_ = kwargs.get('timeout', self.timeout)
...
class Reports(MWS):
    ...
    def get_report(self, report_id, timeout=None):
        data = dict(Action='GetReport', ReportId=report_id)

        return self.make_request(data, timeout=timeout)
